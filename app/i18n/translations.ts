export const translations = {
  en: {
    ...
    hero: {
      h1a: 'Send USDC.',
      h1b: 'No internet.',
      sub: 'Sign a payment with no internet, hand it to the receiver by QR code or Bluetooth, and it settles on Arc in under a second — from whichever phone finds a connection first. Money that moves like cash, settles like a blockchain.',
    ...
    story: {
      ...
      p3: 'Wayamesh removes it from the payment itself. Your phone signs USDC with a hardware-bound key, hands it to the receiver by QR code or Bluetooth, and whichever phone touches the internet first settles it on Arc in under a second. No towers, no banks, no servers.',
      ...
      items: [
        {
          ...
          desc: 'Sign a USDC transfer with no connection. Hand it to the receiver by QR code or Bluetooth — they hold the settlement-ready transaction, with automatic dedup when several phones carry the same payment.',
        },
        {
          title: 'Self-custodial & secure',
          desc: 'Keys are generated on-device and stored hardware-bound — Android Keystore or iOS Secure Enclave — gated by biometrics. They never leave the phone. No backend between you and your money.',
        },
        {
          title: 'Settles on Arc in under a second',
          desc: 'Whichever phone finds a connection first broadcasts the payment. Arc finality is ≈480ms, and with the gasless smart account you pay gas in USDC itself — no separate gas token.',
        },
        {
          title: 'Agent wallet with hard limits',
          desc: 'A spending envelope automation can never exceed — per-transaction, daily, and total caps, allowlists, expiry — enforced on-device by a deterministic policy engine. Standing payments execute fully offline.',
        },
      ],
    },
    how: {
      ...
      steps: [
        {
          title: 'Sign offline',
          desc: 'You create a payment. Your phone signs the USDC transfer with its hardware-bound key — no connection needed. The signed transaction is sealed: amount, recipient, and sender cannot be altered by anyone.',
        },
        {
          title: 'Hand off',
          desc: 'The payment travels to the receiver’s phone as a QR code or over Bluetooth — no infrastructure. The receiver now holds the settlement-ready transaction; they no longer depend on you reconnecting. Any phone can also courier a third party’s payment to the internet.',
        },
        {
          title: 'Settle on Arc',
          desc: 'Whichever phone touches the internet first broadcasts it. Arc finality is about half a second. Once settled it is irreversible — no chargebacks, no cancellation.',
        },
      ],
    },
    mobile: {
    ...
    faq: {
      ...
      items: [
        {
          q: 'Is my money safe?',
          a: 'Your private keys are generated on-device and stored hardware-bound — Android Keystore or iOS Secure Enclave — unlocked with biometrics. Every payment is signed locally, and the signed transaction is sealed: amount, recipient, and sender cannot be altered by anyone. Even if a handoff is intercepted, nobody can access your funds without your key.',
        },
        {
          q: 'Do you hold my funds?',
          a: 'No. Wayamesh is fully self-custodial and has no backend — your phone talks directly to Arc. Only your device holds your private key. No custody, no servers, no middlemen. Recovery works three ways: seed phrase, PIN-protected cloud backup, or Shamir guardian shards.',
        },
        {
          q: 'How do payments work offline?',
          a: 'Your phone signs the USDC transfer with no connection, then hands it to the receiver as a QR code or over Bluetooth — the receiver needs no internet either. They hold the settlement-ready transaction and no longer depend on you reconnecting. Whichever phone touches the internet first broadcasts it, and Arc settles it in about half a second. Duplicates are detected automatically when several phones carry the same payment.',
        },
        {
          q: 'What is the agent wallet?',
          a: 'A spending envelope that AI or automation can never exceed: per-transaction, daily, and total caps, allowlists, and expiry — enforced on-device by a deterministic policy engine with a full audit log. Standing payments execute fully offline: due payments pass the mandate gate, sign locally, and settle at reconnect.',
        },
        {
          q: 'Can it be blocked?',
          a: 'The protocol is open-source (MIT) and peer-to-peer. QR handoff works between any two phones with cameras, and Bluetooth is standard hardware no government can selectively ban. Funds settle on Arc — censorship-resistant by design.',
        },
      ],
    },
  },
